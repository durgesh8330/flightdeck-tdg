...
export class SystemParametersComponent implements OnInit {
	...
	constructor(private userProfileService: UserProfileService, private dialog: MatDialog,
		...
		userProfileService.getPageLayout('System Parameter Table').toPromise().then(res => {
			// debugger;
	...
	getViewDataTable() {
		...
		this.userProfileService.getSystemData("")
			.subscribe(res => {
				// debugger;
			    console.log(res);
				this.systemParameter.tableData = [];
				this.systemParameter.tableData =  res['systemParameterModels'];
				this.pagination.totalRecords = res['systemParameterModels'].length;
	...
	onSaveClick(data) {
		...
		this.httpClient.put(environment.apiUrl+"/Enterprise/v2/Work/systemParameter/"+sysParam.id, sysParam)
			.toPromise().then((response: any) => {
				this.setMessage(true, "Successfully updated system parameter");
	...
	createSystemParameter(pagedItems, rowIndex, requestObj) {
		this.httpClient.post(environment.apiUrl+"/Enterprise/v2/Work/systemParameter", requestObj)
			.toPromise().then((response: any) => {
			pagedItems.splice(rowIndex, 1,response);
			...
			this.setMessage(true, "Created new item with name " + response.name);
		}).catch(error => {
    ...
    onDeleteClick(data) {
		...
		this.modalDetails = { ...this.modalDetails, isDeleteConfirm: true, 
			...
			subTitle: "Are you sure that you want to delete the System Parameter?", 
			from: "deletesystemparameter", data: data };
        this.modalChild.showModal();
    }

	buttonClicked(fromModal) {
        console.log("buttonClicked(fromModal)");
		if(fromModal.modal.from == 'deletesystemparameter') {
          this.deleteSystemParameter(fromModal.modal.data);
        }
	}

    deleteSystemParameter(data) {
        console.log("deleteSystemParameter(index)");
		var pagedItems = data.pagedItems;
		var rowIndex = data.rowIndex;
		var id = pagedItems[rowIndex]['id'];
		if (!id) {
			this.setMessage(false, "System Parameter to be deleted does not have an ID");
			this.modalChild.setErrorMessage(this.message);
			return;
		}
        this.httpClient.delete(environment.apiUrl+'/Enterprise/v2/Work/systemParameter/' + id).
            ...
            .catch(data => { 
				...
				this.setMessage(false, this.message);
				this.modalChild.setErrorMessage(this.message);
             });
	}
	...
	openTab(event) {
		if (event.link) {
			...
			let tab = new Tab(this.getComponentType('SystemParametersDetailsComponent'),  event.name, '/SystemParameter/', {});
