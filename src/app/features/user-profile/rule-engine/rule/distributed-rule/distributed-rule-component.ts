...
import { Resource } from '../../shared/services/rule/resource-field.model';
...
@Component({
  selector: 'app-distributed-rule-component',
  templateUrl: './distributed-rule-component.html',
  styleUrls: ['./distributed-rule-component.scss']
})
export class DistributedRuleComponent implements OnInit, ProcessComponent {
  ...
  constructor(private _formBuilder: FormBuilder, private route: ActivatedRoute, private filterService: FilterService, private ruleInputService: RuleInputService, private dragulaService: DragulaService, private matDialog: MatDialog, private _Activatedroute: ActivatedRoute, private ruleService: RuleService, private toasterService: ToasterService, 
  ...
  ngOnInit() {
    ...
    this.resourceService.getApplications().toPromise().then((response: any) => {
  ...
  checkVisible(filter: any, type: String, isDateType: boolean, filterType: string) {
    let filterField: Resource[];
    if(filterType === 'resource'){
      filterField = this.availResourceFilter.filter((availFiled: Resource) => availFiled.fieldName === filter.value.fieldName);
    }else{
      filterField = this.availTaskFilter.filter((availField: Resource) => availField.fieldName === filter.value.fieldName);
    }
  ...
  getFieldValues(fieldName, filterType: string) {
    // tslint:disable-next-line:no-shadowed-variable
    let filterField: Resource[];
    if(filterType === 'resource'){
      filterField = this.availResourceFilter.filter((availField: Resource) => availField.fieldName === fieldName);
    }else{
      filterField = this.availTaskFilter.filter((availField: Resource) => availField.fieldName === fieldName);
    }
  ...
  createOrUpdateRule() {
    ...
    request.applicationName = this.applicationControl.value;
  ...
  validateGeneralSetting(stepper: MatStepper){
    if(this.applicationControl.value && this.generalSettingsFormGroup.value.ruleName){
      stepper.next();
    }
  }

  validateResourceSetting(stepper: MatStepper){
    if(this.resourceFilters && this.resourceFilters.controls.length > 0){
        let isFilterValid = true;
        for(let i=0; i<this.resourceFilters.controls.length; i++){
          const filter = this.resourceFilters.controls[i];
          if(!filter.value || !filter.value.fieldName || !filter.value.fieldOperator || !filter.value.fieldValue){
            isFilterValid = false;
              break;
          }
        }
      if(isFilterValid){
        ...
        this.toasterService.pop('error', 'Please select the value for filters');
      }
    }
  }
  ...
  getComponentData(): void {
      ...
      forkJoin([this.filterService.getResourceFields(), this.filterService.getTaskFields(),
              this.ruleInputService.getAllRuleInputs()]).subscribe(results => {
